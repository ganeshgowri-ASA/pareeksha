...
export default function ReliabilityPlanner() {
  ...
  const standard = getStandard(selectedStandard) ?? STANDARDS[0];

  // Flatten sequences into tests
  const flatTests = standard.sequences.flatMap((seq) =>
    seq.tests.map((t) => {
  ...
  const computedTests = tests.map((t) => {
    ...
    const slots = chamber?.slotsFullSize ?? 1;
    const totalHours = t.annualDemand * t.samplesRequired * t.testHours;
    const capacity = slots * calculationInput.workHoursPerYear * calculationInput.realisationRate;
            ...
            {computedTests.map((t) => {
              ...
              return (
                <tr key={t.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-600">{chamber?.name ?? t.chamberType}</td>
